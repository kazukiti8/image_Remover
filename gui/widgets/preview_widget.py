...
# === ズーム・パン機能付き QGraphicsView (変更なし) ===
class ZoomPanGraphicsView(QGraphicsView):
    """マウスホイールでズーム、ドラッグでパンできる QGraphicsView"""
    clicked = Signal()
    ...
    def set_image(self, pixmap: Optional[QPixmap]) -> None:
        ...
        if pixmap and not pixmap.isNull():
            ...
            self.resetTransform()
            self.setSceneRect(self.rect().adjusted(0,0,-2,-2))
            self.initial_label.setVisible(True)
            self.initial_label.setGeometry(self.rect().adjusted(2,2,-2,-2))
    ...
    def resizeEvent(self, event) -> None:
        ...
        if self.initial_label.isVisible():
            self.initial_label.setGeometry(self.rect().adjusted(2,2,-2,-2))
...
class PreviewWidget(QWidget):
    ...
    def _setup_ui(self) -> None:
        ...
        self.left_preview_view.setToolTip("クリックで削除 / Aキーで開く\nホイールでズーム、ドラッグでパン")
        ...
        self.right_preview_view.setToolTip("クリックで削除 / Sキーで開く\nホイールでズーム、ドラッグでパン")
        ...
        self.diff_checkbox.setToolTip("チェックを入れると、右側のプレビューに左右画像の差分を表示します。\n画像サイズが異なる場合は無効です。")
    ...
    def _load_image_and_get_size(self, image_path: str, mode: str = 'rgb') -> LoadResult:
        ...
        if img_np is not None and error_msg is None:
            try:
                h, w = img_np.shape[:2]
                return img_np, None, (w, h)
            except Exception as e:
                return None, f"サイズ取得エラー: {e}", None
    ...
    def _calculate_difference(self, img1_bgr: NumpyImageType, img2_bgr: NumpyImageType) -> Optional[NumpyImageType]:
        """2つのBGR画像の差分を計算する (同サイズのみ)"""
        if img1_bgr.shape != img2_bgr.shape:
            print("差分計算スキップ: 画像サイズが異なります。")
            return None
        try:
            diff = cv2.absdiff(img1_bgr, img2_bgr)
            return diff
        except cv2.error as e:
            print(f"差分計算エラー (OpenCV): {e}")
            return None
        except Exception as e:
            print(f"差分計算エラー: {e}")
            return None
    ...
    def _numpy_to_pixmap(self, img_np: NumpyImageType) -> Optional[QPixmap]:
        """NumPy配列 (BGRまたはGrayscale) を QPixmap に変換する"""
        if img_np is None: return None
        try:
            qt_image: QImage
            if len(img_np.shape) == 3: # BGR
                h, w, ch = img_np.shape
                bytes_per_line = ch * w
                qt_image = QImage(cv2.cvtColor(img_np, cv2.COLOR_BGR2RGB).data, w, h, bytes_per_line, QImage.Format.Format_RGB888).copy()
            elif len(img_np.shape) == 2: # Grayscale
                h, w = img_np.shape
                bytes_per_line = w
                qt_image = QImage(img_np.data, w, h, bytes_per_line, QImage.Format.Format_Grayscale8).copy()
            else:
                print("未対応のNumpy配列形式です。")
                return None
            return QPixmap.fromImage(qt_image)
        except Exception as e:
            print(f"NumPyからPixmapへの変換エラー: {e}")
            return None

    def _display_image(self, target_view: ZoomPanGraphicsView, image_path: Optional[str], label_name: str) -> None:
        """指定された QGraphicsView に画像を表示し、画像サイズを記録する"""
        target_view.clear_image()
        current_size: Optional[Tuple[int, int]] = None

        if image_path and os.path.exists(image_path):
            img_bgr: Optional[NumpyImageType]
            error_msg: ErrorMsgType
            img_bgr, error_msg, img_size = self._load_image_and_get_size(image_path, mode='bgr')

            if error_msg:
                print(f"プレビュー画像読込エラー ({os.path.basename(image_path)}): {error_msg}")
                target_view.initial_label.setText(f"{label_name}\n(読込エラー)")
        ...
        if target_view == self.left_preview_view:
            self.left_image_size = current_size
        elif target_view == self.right_preview_view:
            self.right_image_size = current_size
    ...
    def _display_difference(self) -> None:
        ...
        if diff_img is not None:
            ...
            if diff_pixmap:
                ...
                self.right_preview_view.initial_label.setText("差分画像")
            else:
                self.right_preview_view.initial_label.setText("右プレビュー\n(差分表示エラー)")
                self.right_preview_view.clear_image()
        else:
    ...
    def update_previews(self, left_path: Optional[str], right_path: Optional[str]) -> None:
        """左右のプレビュー画像を更新する"""
        left_changed = self.left_image_path != left_path
        right_changed = self.right_image_path != right_path
        ...
        if self.diff_checkbox.isChecked() and self.diff_checkbox.isEnabled():
            self._display_difference()
