...
from PySide6.QtWidgets import (
    ...
    QDialogButtonBox, QCheckBox, QGroupBox, QWidget
)
...
class SettingsDialog(QDialog):
    ...
    def __init__(self, current_settings: SettingsDict, parent: Optional[QWidget] = None):
        ...
        # ウィジェットの型ヒント
        self.scan_subdirectories_checkbox: QCheckBox # ★ 追加 ★
    ...
    def _setup_ui(self) -> None:
        ...
        # --- 一般設定 ---
        general_group = QGroupBox("スキャン設定") # グループ追加
        ...
        self.scan_subdirectories_checkbox.setChecked(bool(self.settings.get('scan_subdirectories', False))) # デフォルトはFalse
        ...
        self.use_phash_checkbox = QCheckBox("pHashによる候補絞り込みを行う"); self.use_phash_checkbox.setToolTip("有効にすると、最初にpHashで候補を絞り込み、処理速度が向上する場合があります。\nImageHashライブラリが必要です。"); self.use_phash_checkbox.setChecked(bool(self.settings.get('use_phash', True)))
        self.hash_threshold_label = QLabel("pHash ハミング距離閾値:"); self.hash_threshold_spinbox = QSpinBox(); self.hash_threshold_spinbox.setRange(0, 64); self.hash_threshold_spinbox.setSingleStep(1); self.hash_threshold_spinbox.setValue(int(self.settings.get('hash_threshold', 5)))
        ...
        self.orb_features_label = QLabel("ORB 特徴点数:"); self.orb_features_spinbox = QSpinBox(); self.orb_features_spinbox.setRange(100, 10000); self.orb_features_spinbox.setSingleStep(100); self.orb_features_spinbox.setValue(int(self.settings.get('orb_nfeatures', 1500)))
        self.orb_ratio_label = QLabel("ORB Ratio Test 閾値:"); self.orb_ratio_spinbox = QDoubleSpinBox(); self.orb_ratio_spinbox.setRange(0.1, 0.95); self.orb_ratio_spinbox.setSingleStep(0.01); self.orb_ratio_spinbox.setDecimals(2); self.orb_ratio_spinbox.setValue(float(self.settings.get('orb_ratio_threshold', 0.70)))
        self.orb_min_matches_label = QLabel("類似ペア判定 最小マッチ数:"); self.orb_min_matches_spinbox = QSpinBox(); self.orb_min_matches_spinbox.setRange(5, 500); self.orb_min_matches_spinbox.setSingleStep(1); self.orb_min_matches_spinbox.setValue(int(self.settings.get('min_good_matches', 40)))
    ...
    def accept(self) -> None:
        """OKボタンが押されたときの処理"""
        self.settings['scan_subdirectories'] = self.scan_subdirectories_checkbox.isChecked() # ★ 追加 ★
